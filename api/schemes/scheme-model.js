...
async function findById(scheme_id) { // EXERCISE B
  ...
  rows.forEach(row => {
    if(row.step_id){
      editedData.steps.push({step_id: row.step_id, step_number: row.step_number, instructions:row.instructions})
    }
  })
  ...
  return editedData
    ...
    //     'scheme_name': editedData.scheme_name,
    //     'steps': []
    //   }
    //    data.map(step => {
    //     editedData.steps.push(step.step_id)
    //   })
    //   return editedData
    // }
...
async function findSteps(scheme_id) { 
  ...
  rows.forEach(row => {
    if(row.step_id){
      steps.push({step_id: row.step_id, step_number: row.step_number, instructions:row.instructions, scheme_name: row.scheme_name})
    }
  })
